#!/usr/bin/env bash
# Stage only: remotely-managed tunnel ingress AND proxied CNAME for *-stage.racunai.hr.
# Merges other hostnames on a shared tunnel. Never touches production hosts.
set -euo pipefail
...
load_env_file /opt/stacks/traefik/.env
load_env_file "${ROOT_DIR}/scripts/.env"
load_env_file "${ROOT_DIR}/cloudflared/.env"

MODE="stage"
CF_API="https://api.cloudflare.com/client/v4"
ZONE_NAME="${CLOUDFLARE_ZONE_NAME:-racunai.hr}"
NAMES="${RACUNAI_DNS_NAMES:-$STAGE_DNS_ALLOWLIST}"
TOKEN="${CF_DNS_TOKEN_STAGE:-${CF_DNS_API_TOKEN:-}}"
TUNNEL_ID="${CLOUDFLARE_TUNNEL_ID:-}"
ACCOUNT_ID="${CLOUDFLARE_ACCOUNT_ID:-34684c48969ce0a4dbab07e5d2a5c8f6}"
INGRESS_SERVICE="${RACUNAI_TUNNEL_SERVICE:-http://traefik:80}"

[[ -n "$TOKEN" ]] || die "CF_DNS_TOKEN_STAGE or CF_DNS_API_TOKEN is required"
[[ -n "$TUNNEL_ID" ]] || die "CLOUDFLARE_TUNNEL_ID is required"

for name in $NAMES; do
  assert_allowlist "$MODE" "$name"
  assert_stage_suffix "$name"
done

if [[ "${RACUNAI_DRY_RUN:-}" == "1" ]]; then
